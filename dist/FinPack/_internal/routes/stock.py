...
def get_industry_data():
    ...
    一次性返回所有日期的 Sharpe 和 Growth 矩陣 + 預計算排名
    
    Query Parameters:
        period: 時間範圍 (3mo, 6mo, 1y)
    """
    container = get_container()
...
def get_backtest_prices():
    ...
    返回所有股票在時間範圍內的價格矩陣，包含指數數據供 benchmark 使用
    
    Query Parameters:
        period: 時間範圍 (3mo, 6mo, 1y, 2y, 5y, 6y)
    """
    container = get_container()
