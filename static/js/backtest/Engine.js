...
export class BacktestEngine {
    ...
    async run(data, options = {}) {
        ...
        const { dates, prices, stockInfo, sharpeRank, growthRank, sharpeValues, growthValues } = data;
        ...
        try {
            for (let i = 0; i < tradingDates.length; i++) {
                ...
                // 5. 記錄每日權益（傳入 stockInfo 以記錄每日持有詳情）
                this.portfolio.recordHistory(date, context.prices, context.stockInfo);
                ...
                if (onProgress) {
                    onProgress({
                        ...
                        date,
                        equity: this.portfolio.calculateValue(context.prices).totalValue
                    });
                }
                
                // 保存最後一天的價格供 finalPositions 使用
    ...
    async _processSellSignals(date, context) {
        ...
        for (const ticker of holdings) {
            ...
            if (sellCheck.shouldSell) {
                ...
                this.trade.executeSell(ticker, price, date, reasons);
            }
        }
    }
    ...
    async _processBuySignals(date, selectedStocks, context) {
        ...
        for (const ticker of toBuy) {
            ...
            if (price !== undefined && info) {
                this.trade.executeBuy(ticker, price, info.country, date);
            }
        }
    }
